...
#define I2C_IO_SDA GPIO_NUM_4
#define I2C_IO_SCL GPIO_NUM_5

#define I2C_MASTER_FREQ_HZ 1200000 /*!< I2C master clock frequency. no higher than 1MHz for now */
SSD1306_t dev1306;
...
void i2c_master_init(void)
{
	...
	dev1306._address = I2CAddress;
	dev1306._flip = false;
}

void i2c_init(void) {

	dev1306._width = SSD1306_WIDTH;
	dev1306._height = SSD1306_HEIGHT;
	dev1306._pages = 8;
	if (dev1306._height == 32) dev1306._pages = 4;
	
	i2c_cmd_handle_t cmd = i2c_cmd_link_create();

	i2c_master_start(cmd);
	i2c_master_write_byte(cmd, (dev1306._address << 1) | I2C_MASTER_WRITE, true);
	...
	if (dev1306._height == 64) i2c_master_write_byte(cmd, 0x3F, true);
	if (dev1306._height == 32) i2c_master_write_byte(cmd, 0x1F, true);
	...
	if (dev1306._height == 64) i2c_master_write_byte(cmd, 0x12, true);
	if (dev1306._height == 32) i2c_master_write_byte(cmd, 0x02, true);
...
void i2c_hardware_scroll(ssd1306_scroll_type_t scroll) {
	esp_err_t espRc;
	...
	if (scroll == SCROLL_DOWN) {
		...
		i2c_master_write_byte(cmd, 0x00, true);
		if (dev1306._height == 64)
		//i2c_master_write_byte(cmd, 0x7F, true);
		i2c_master_write_byte(cmd, 0x40, true);
		if (dev1306._height == 32)
